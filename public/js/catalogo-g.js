/**
 * Catálogo G — UI interactiva (grid/lista, filtros, modal, Excel).
 */
(function () {
  ...
  const state = {
    ...
    filtroEstado: '',
    filtroBodega: '',
    view: localStorage.getItem('cg_view') || 'list',
    editId: null,
  ...
  function paint() {
    ...
    content.innerHTML = `
      <div class="cg-page">
        ...
        ${!state.canEdit ? `<div class="alert alert-info" style="margin-bottom:.85rem">Solo lectura. Edición: Admin / Subadmin, rol Catálogo G o permiso editor.</div>` : ''}
        ${catalogHtml(rows)}
        ${modalHtml()}
  ...
  async function downloadPlantilla() {
    ...
    const res = await fetch('/api/modulos/catalogo-g/plantilla', {
    ...
    a.download = 'plantilla_catalogo_g.xlsx';
    a.click();
  ...
  async function importExcel(file) {
    ...
    const res = await Auth.api('/api/modulos/catalogo-g/import-excel', {
      method: 'POST',
  ...
  async function saveItem(item) {
    ...
    try {
      if (state.editId) {
        await Auth.api('/api/modulos/catalogo-g/' + state.editId, { method: 'POST', body: JSON.stringify(body) });
        UI.flash('Ítem actualizado');
      } else {
        const r = await Auth.api('/api/modulos/catalogo-g', { method: 'POST', body: JSON.stringify(body) });
    ...
    state.forzarFotoDup = false;
    state.view = 'list';
    localStorage.setItem('cg_view', 'list');
    await boot(false);
  }
  ...
  function bind() {
    ...
    document.querySelectorAll('[data-view]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        localStorage.setItem('cg_view', state.view);
        paint();
      });
    });
    ...
    document.querySelectorAll('[data-del]').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        if (!confirm('¿Eliminar este ítem del Catálogo G?')) return;
        try {
          await Auth.api('/api/modulos/catalogo-g/' + id + '/eliminar', { method: 'POST', body: '{}' });
          UI.flash('Eliminado');
          await boot(false);
        } catch (err) {
      ...
      async function addFiles(tipo, fileList) {
        ...
        for (const file of files.slice(0, room)) {
          ...
          try {
            ...
            if (hash) {
              try {
                const check = await Auth.api('/api/modulos/catalogo-g/check-foto', {
                  method: 'POST',
        ...
        if (lastCompressed && !lastDup) {
          ...
          try {
            const sug = await Auth.api('/api/modulos/catalogo-g/sugerir-foto', {
              method: 'POST',
  ...
  async function boot(showShellError) {
    ...
    if (String(user?.empresa || '').toLowerCase() !== 'global') {
      content.innerHTML = `<div class="alert alert-error">Catálogo G solo está disponible para la empresa Global.</div>`;
      return;
    }
    try {
      const res = await Auth.api('/api/modulos/catalogo-g');
