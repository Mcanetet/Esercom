/**
 * Catálogo S — inventario Sercom (empresa, producto, bodega, foto, estado).
 */
const fs = require('fs');
...
const UPLOAD_DIR = path.join(config.publicDir, 'uploads', 'catalogo-s');
const TABLE = 'catalogo_s';
...
async function ensureCatalogoSSchema(db) {
  if (db.driver === 'mysql') {
    await db.exec(`
...
async function backfillFotosJson(db) {
  try {
    ...
    console.warn('[catalogo-s] backfill fotos:', err.message);
  }
}
...
async function ensureColumn(db, col, mysqlDdl, sqliteDdl) {
  try {
    ...
    if (!/duplicate column/i.test(String(err.message || ''))) {
      console.warn(`[catalogo-s] columna ${col}:`, err.message);
    }
  }
}
...
async function backfillCorrelativos(db) {
  try {
    ...
    for (const r of rows) {
      ...
      const codigo = `CATS-${String(n).padStart(5, '0')}`;
      await db.prepare(`
        UPDATE ${TABLE}
    ...
    console.warn('[catalogo-s] backfill correlativo:', err.message);
  }
}

/** Rol «Catálogo S» con acceso al módulo (idempotente). */
async function ensureCatalogoSRole(db) {
  const pages = ['home.html', 'catalogo-s.html'];
  const pagesJson = JSON.stringify(pages);
  const nombre = 'Catálogo S';
  const descripcion = 'Acceso al módulo Catálogo S (empresa Sercom). Puede crear y editar ítems.';
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn('[catalogo-s] no se pudo leer roles:', e.message);
        return;
      }
    }
  }
  ...
  const existing = (rows || []).find((r) => {
    ...
    return n === 'catalogo s' || n === 'catalogo-s' || /^catalogo\s*s$/.test(n);
  });
  ...
  if (existing) {
    ...
    const has = cur.some((p) => String(p).replace(/^\//, '') === 'catalogo-s.html');
    if (has) return;
    const next = JSON.stringify([...new Set([...cur, 'home.html', 'catalogo-s.html'])]);
    try {
      ...
      try {
        ...
        console.warn('[catalogo-s] no se pudo actualizar rol:', e.message);
      }
    }
    return;
  }
  ...
  try {
    ...
    try {
      ...
      if (!/duplicate|unique/i.test(String(e.message || ''))) {
        console.warn('[catalogo-s] no se pudo crear rol:', e.message);
      }
    }
  }
}
...
function formatCodigo(n) {
  return `CATS-${String(n).padStart(5, '0')}`;
}
...
async function analyzePhotoWithAngel(db, dataUrl, candidates, timeoutMs = 8000) {
  ...
  const work = async () => {
    ...
    const content = [
      {
        type: 'text',
        text: `Eres Angel IA de ESERCOM. Compara la FOTO NUEVA con fotos existentes del Catálogo S.
Responde SOLO JSON:
{"duplicada":true|false,"coincidencias":[{"codigo":"CATS-00001","motivo":"..."}],"mensaje":"alerta breve en español"}
Candidatos: ${candidates.map((c) => c.codigo).join(', ')}`
  ...
  try {
    ...
    console.warn('[catalogo-s] angel foto check:', err.message);
    return null;
  }
}
 ...
 */
async function suggestFromCatalogoSPhoto(db, dataUrl, opts = {}) {
    ...
    const resp = await client.chat.completions.create({
      ...
      messages: [{
        ...
        content: [
          {
            type: 'text',
            text: `Eres Angel IA de ESERCOM (inventario Catálogo S, Chile).
...
- No inventes códigos ni precios.`
  ...
  try {
    ...
    console.warn('[catalogo-s] sugerir foto:', err.message);
 ...
 */
async function analyzeCatalogoSPhoto(db, dataUrl, opts = {}) {
...
async function listCatalogoS(db) {
...
async function createCatalogoS(db, userId, body) {
  await ensureCatalogoSSchema(db);
...
async function updateCatalogoS(db, id, userId, body) {
  await ensureCatalogoSSchema(db);
...
async function deleteCatalogoS(db, id, userId) {
  await ensureCatalogoSSchema(db);
  const info = await db.prepare(`
...
function saveCatalogoSPhoto(dataUrl, analysis) {
  const parsed = analysis || parseDataUrl(dataUrl);
  const { ext, buf, hash } = parsed;
  if (!fs.existsSync(UPLOAD_DIR)) fs.mkdirSync(UPLOAD_DIR, { recursive: true });
  const name = `cats_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`;
  fs.writeFileSync(path.join(UPLOAD_DIR, name), buf);
  return { ruta: `/uploads/catalogo-s/${name}`, hash };
}

/** Bodegas del módulo Salida de Materiales (+ defaults). */
async function listBodegasCatalogoS(db) {
  try {
    const rows = await db.prepare(`
...
async function buildPlantillaCatalogoS(bodegas) {
  ...
  ws.addRow(['Sercom', 'Ejemplo producto acero', 10, bod, 'Nuevo']);
  ws.addRow(['Sercom', 'Ejemplo herramienta usada', 2, bod, 'Usado']);
...
async function importCatalogoSExcel(db, buffer, userId, opts = {}) {
  await ensureCatalogoSSchema(db);
  const wb = new ExcelJS.Workbook();
  ...
  for (const ws of wb.worksheets || []) {
    ...
    for (const row of extractCatalogoRows(ws)) {
      ...
      const emp = String(row.empresa || '').trim() || 'Sercom';
      if (!desc) continue;
  ...
  for (const row of rows) {
    ...
    await createCatalogoS(db, userId, {
      empresa: row.empresa || 'Sercom',
  ...
  return {
    total: created,
    archivo_nombre: String(opts.filename || 'catalogo_s.xlsx').slice(0, 255)
  };
}
...
module.exports = {
  ESTADOS,
  BODEGAS_DEFAULT,
  ensureCatalogoSSchema,
  ensureCatalogoSRole,
  listCatalogoS,
  listBodegasCatalogoS,
  buildStats,
  createCatalogoS,
  updateCatalogoS,
  deleteCatalogoS,
  saveCatalogoSPhoto,
  analyzeCatalogoSPhoto,
  suggestFromCatalogoSPhoto,
  findDuplicatesByHash,
  normalizeFotosList,
  primaryFoto,
  buildPlantillaCatalogoS,
  importCatalogoSExcel,
  decodeUploadBody
};
