/**
 * Catálogo N — inventario Nexus (empresa, producto, bodega, foto, estado).
 */
const fs = require('fs');
...
const UPLOAD_DIR = path.join(config.publicDir, 'uploads', 'catalogo-n');
const TABLE = 'catalogo_n';
...
async function ensureCatalogoNSchema(db) {
  if (db.driver === 'mysql') {
    await db.exec(`
...
async function backfillFotosJson(db) {
  try {
    ...
    console.warn('[catalogo-n] backfill fotos:', err.message);
  }
}
...
async function ensureColumn(db, col, mysqlDdl, sqliteDdl) {
  try {
    ...
    if (!/duplicate column/i.test(String(err.message || ''))) {
      console.warn(`[catalogo-n] columna ${col}:`, err.message);
    }
  }
}
...
async function backfillCorrelativos(db) {
  try {
    ...
    for (const r of rows) {
      ...
      const codigo = `CATN-${String(n).padStart(5, '0')}`;
      await db.prepare(`
        UPDATE ${TABLE}
    ...
    console.warn('[catalogo-n] backfill correlativo:', err.message);
  }
}

/** Rol «Catálogo N» con acceso al módulo (idempotente). */
async function ensureCatalogoNRole(db) {
  const pages = ['home.html', 'catalogo-n.html'];
  const pagesJson = JSON.stringify(pages);
  const nombre = 'Catálogo N';
  const descripcion = 'Acceso al módulo Catálogo N (empresa Nexus). Puede crear y editar ítems.';
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn('[catalogo-n] no se pudo leer roles:', e.message);
        return;
      }
    }
  }
  ...
  const existing = (rows || []).find((r) => {
    ...
    return n === 'catalogo n' || n === 'catalogo-n' || /^catalogo\s*n$/.test(n);
  });
  ...
  if (existing) {
    ...
    const has = cur.some((p) => String(p).replace(/^\//, '') === 'catalogo-n.html');
    if (has) return;
    const next = JSON.stringify([...new Set([...cur, 'home.html', 'catalogo-n.html'])]);
    try {
      ...
      try {
        ...
        console.warn('[catalogo-n] no se pudo actualizar rol:', e.message);
      }
    }
    return;
  }
  ...
  try {
    ...
    try {
      ...
      if (!/duplicate|unique/i.test(String(e.message || ''))) {
        console.warn('[catalogo-n] no se pudo crear rol:', e.message);
      }
    }
  }
}
...
function formatCodigo(n) {
  return `CATN-${String(n).padStart(5, '0')}`;
}
...
async function analyzePhotoWithAngel(db, dataUrl, candidates, timeoutMs = 8000) {
  ...
  const work = async () => {
    ...
    const content = [
      {
        type: 'text',
        text: `Eres Angel IA de ESERCOM. Compara la FOTO NUEVA con fotos existentes del Catálogo N.
Responde SOLO JSON:
{"duplicada":true|false,"coincidencias":[{"codigo":"CATN-00001","motivo":"..."}],"mensaje":"alerta breve en español"}
Candidatos: ${candidates.map((c) => c.codigo).join(', ')}`
  ...
  try {
    ...
    console.warn('[catalogo-n] angel foto check:', err.message);
    return null;
  }
}
 ...
 */
async function suggestFromCatalogoNPhoto(db, dataUrl, opts = {}) {
    ...
    const resp = await client.chat.completions.create({
      ...
      messages: [{
        ...
        content: [
          {
            type: 'text',
            text: `Eres Angel IA de ESERCOM (inventario Catálogo N, Chile).
...
- No inventes códigos ni precios.`
  ...
  try {
    ...
    console.warn('[catalogo-n] sugerir foto:', err.message);
 ...
 */
async function analyzeCatalogoNPhoto(db, dataUrl, opts = {}) {
...
async function listCatalogoN(db) {
...
async function createCatalogoN(db, userId, body) {
  await ensureCatalogoNSchema(db);
...
async function updateCatalogoN(db, id, userId, body) {
  await ensureCatalogoNSchema(db);
...
async function deleteCatalogoN(db, id, userId) {
  await ensureCatalogoNSchema(db);
  const info = await db.prepare(`
...
function saveCatalogoNPhoto(dataUrl, analysis) {
  const parsed = analysis || parseDataUrl(dataUrl);
  const { ext, buf, hash } = parsed;
  if (!fs.existsSync(UPLOAD_DIR)) fs.mkdirSync(UPLOAD_DIR, { recursive: true });
  const name = `catn_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`;
  fs.writeFileSync(path.join(UPLOAD_DIR, name), buf);
  return { ruta: `/uploads/catalogo-n/${name}`, hash };
}

/** Bodegas del módulo Salida de Materiales (+ defaults). */
async function listBodegasCatalogoN(db) {
  try {
    const rows = await db.prepare(`
...
async function buildPlantillaCatalogoN(bodegas) {
  ...
  ws.addRow(['Nexus', 'Ejemplo producto acero', 10, bod, 'Nuevo']);
  ws.addRow(['Nexus', 'Ejemplo herramienta usada', 2, bod, 'Usado']);
...
async function importCatalogoNExcel(db, buffer, userId, opts = {}) {
  await ensureCatalogoNSchema(db);
  const wb = new ExcelJS.Workbook();
  ...
  for (const ws of wb.worksheets || []) {
    ...
    for (const row of extractCatalogoRows(ws)) {
      ...
      const emp = String(row.empresa || '').trim() || 'Nexus';
      if (!desc) continue;
  ...
  for (const row of rows) {
    ...
    await createCatalogoN(db, userId, {
      empresa: row.empresa || 'Nexus',
  ...
  return {
    total: created,
    archivo_nombre: String(opts.filename || 'catalogo_n.xlsx').slice(0, 255)
  };
}
...
module.exports = {
  ESTADOS,
  BODEGAS_DEFAULT,
  ensureCatalogoNSchema,
  ensureCatalogoNRole,
  listCatalogoN,
  listBodegasCatalogoN,
  buildStats,
  createCatalogoN,
  updateCatalogoN,
  deleteCatalogoN,
  saveCatalogoNPhoto,
  analyzeCatalogoNPhoto,
  suggestFromCatalogoNPhoto,
  findDuplicatesByHash,
  normalizeFotosList,
  primaryFoto,
  buildPlantillaCatalogoN,
  importCatalogoNExcel,
  decodeUploadBody
};
