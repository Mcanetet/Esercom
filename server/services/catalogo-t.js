/**
 * Catálogo T — inventario Táctica (empresa, producto, bodega, foto, estado).
 */
const fs = require('fs');
...
const UPLOAD_DIR = path.join(config.publicDir, 'uploads', 'catalogo-t');
const TABLE = 'catalogo_t';
...
async function ensureCatalogoTSchema(db) {
  if (db.driver === 'mysql') {
    await db.exec(`
...
async function backfillFotosJson(db) {
  try {
    ...
    console.warn('[catalogo-t] backfill fotos:', err.message);
  }
}
...
async function ensureColumn(db, col, mysqlDdl, sqliteDdl) {
  try {
    ...
    if (!/duplicate column/i.test(String(err.message || ''))) {
      console.warn(`[catalogo-t] columna ${col}:`, err.message);
    }
  }
}
...
async function backfillCorrelativos(db) {
  try {
    ...
    for (const r of rows) {
      ...
      const codigo = `CATT-${String(n).padStart(5, '0')}`;
      await db.prepare(`
        UPDATE ${TABLE}
    ...
    console.warn('[catalogo-t] backfill correlativo:', err.message);
  }
}

/** Rol «Catálogo T» con acceso al módulo (idempotente). */
async function ensureCatalogoTRole(db) {
  const pages = ['home.html', 'catalogo-t.html'];
  const pagesJson = JSON.stringify(pages);
  const nombre = 'Catálogo T';
  const descripcion = 'Acceso al módulo Catálogo T (empresa Táctica). Puede crear y editar ítems.';
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn('[catalogo-t] no se pudo leer roles:', e.message);
        return;
      }
    }
  }
  ...
  const existing = (rows || []).find((r) => {
    ...
    return n === 'catalogo t' || n === 'catalogo-t' || /^catalogo\s*t$/.test(n);
  });
  ...
  if (existing) {
    ...
    const has = cur.some((p) => String(p).replace(/^\//, '') === 'catalogo-t.html');
    if (has) return;
    const next = JSON.stringify([...new Set([...cur, 'home.html', 'catalogo-t.html'])]);
    try {
      ...
      try {
        ...
        console.warn('[catalogo-t] no se pudo actualizar rol:', e.message);
      }
    }
    return;
  }
  ...
  try {
    ...
    try {
      ...
      if (!/duplicate|unique/i.test(String(e.message || ''))) {
        console.warn('[catalogo-t] no se pudo crear rol:', e.message);
      }
    }
  }
}
...
function formatCodigo(n) {
  return `CATT-${String(n).padStart(5, '0')}`;
}
...
async function analyzePhotoWithAngel(db, dataUrl, candidates, timeoutMs = 8000) {
  ...
  const work = async () => {
    ...
    const content = [
      {
        type: 'text',
        text: `Eres Angel IA de ESERCOM. Compara la FOTO NUEVA con fotos existentes del Catálogo T.
Responde SOLO JSON:
{"duplicada":true|false,"coincidencias":[{"codigo":"CATT-00001","motivo":"..."}],"mensaje":"alerta breve en español"}
Candidatos: ${candidates.map((c) => c.codigo).join(', ')}`
  ...
  try {
    ...
    console.warn('[catalogo-t] angel foto check:', err.message);
    return null;
  }
}
 ...
 */
async function suggestFromCatalogoTPhoto(db, dataUrl, opts = {}) {
    ...
    const resp = await client.chat.completions.create({
      ...
      messages: [{
        ...
        content: [
          {
            type: 'text',
            text: `Eres Angel IA de ESERCOM (inventario Catálogo T, Chile).
...
- No inventes códigos ni precios.`
  ...
  try {
    ...
    console.warn('[catalogo-t] sugerir foto:', err.message);
 ...
 */
async function analyzeCatalogoTPhoto(db, dataUrl, opts = {}) {
...
async function listCatalogoT(db) {
...
async function createCatalogoT(db, userId, body) {
  await ensureCatalogoTSchema(db);
...
async function updateCatalogoT(db, id, userId, body) {
  await ensureCatalogoTSchema(db);
...
async function deleteCatalogoT(db, id, userId) {
  await ensureCatalogoTSchema(db);
  const info = await db.prepare(`
...
function saveCatalogoTPhoto(dataUrl, analysis) {
  const parsed = analysis || parseDataUrl(dataUrl);
  const { ext, buf, hash } = parsed;
  if (!fs.existsSync(UPLOAD_DIR)) fs.mkdirSync(UPLOAD_DIR, { recursive: true });
  const name = `catt_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`;
  fs.writeFileSync(path.join(UPLOAD_DIR, name), buf);
  return { ruta: `/uploads/catalogo-t/${name}`, hash };
}

/** Bodegas del módulo Salida de Materiales (+ defaults). */
async function listBodegasCatalogoT(db) {
  try {
    const rows = await db.prepare(`
...
async function buildPlantillaCatalogoT(bodegas) {
  ...
  ws.addRow(['Táctica', 'Ejemplo producto acero', 10, bod, 'Nuevo']);
  ws.addRow(['Táctica', 'Ejemplo herramienta usada', 2, bod, 'Usado']);
...
async function importCatalogoTExcel(db, buffer, userId, opts = {}) {
  await ensureCatalogoTSchema(db);
  const wb = new ExcelJS.Workbook();
  ...
  for (const ws of wb.worksheets || []) {
    ...
    for (const row of extractCatalogoRows(ws)) {
      ...
      const emp = String(row.empresa || '').trim() || 'Táctica';
      if (!desc) continue;
  ...
  for (const row of rows) {
    ...
    await createCatalogoT(db, userId, {
      empresa: row.empresa || 'Táctica',
  ...
  return {
    total: created,
    archivo_nombre: String(opts.filename || 'catalogo_t.xlsx').slice(0, 255)
  };
}
...
module.exports = {
  ESTADOS,
  BODEGAS_DEFAULT,
  ensureCatalogoTSchema,
  ensureCatalogoTRole,
  listCatalogoT,
  listBodegasCatalogoT,
  buildStats,
  createCatalogoT,
  updateCatalogoT,
  deleteCatalogoT,
  saveCatalogoTPhoto,
  analyzeCatalogoTPhoto,
  suggestFromCatalogoTPhoto,
  findDuplicatesByHash,
  normalizeFotosList,
  primaryFoto,
  buildPlantillaCatalogoT,
  importCatalogoTExcel,
  decodeUploadBody
};
